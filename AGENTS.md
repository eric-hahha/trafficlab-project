...
透過 `scripts/run_inference.py`（`trafficlab.inference.pipeline.InferencePipeline`）執行 YOLO 偵測＋追蹤＋bbox 投影，是 TrafficLab 原生的定位方法，不使用關鍵點。這是兩種定位方法之一（另一種是下方的「keypoints-openpifpaf」）— 依情境挑選合適的方法，不要把它當成預設要跑的方法。
