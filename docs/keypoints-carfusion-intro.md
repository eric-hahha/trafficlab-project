...
results = model.track(frame, conf=0.25, persist=True,
                       tracker='trafficlab/inference/bytetrack.yaml', verbose=False)
result  = results[0]
...
# 跨幀持續的 track id；這幀沒被 tracker 接受的框整個不會出現在 result.boxes 裡
...
| `--frames` | -1 (全部) | 最多處理幾幀 |
| `--conf` | 0.25 | YOLO 偵測閾值（進入 tracker 前的第一道過濾，見「跨幀追蹤」） |
| `--kp-conf` | 0.2 | 關鍵點信心閾值 |
| `--tracker` | `trafficlab/inference/bytetrack.yaml` | ByteTrack 設定檔（pinned 進專案，非套件內建預設）|
