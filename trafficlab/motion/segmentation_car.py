...
def polygon_tight_bbox(polygon) -> tuple[float, float, float, float] | None:
    """Axis-aligned extents (min_x, min_y, max_x, max_y) of a [[x,y], ...]
    polygon; None if the polygon is empty. Split out from mask_to_tight_bbox
    so a caller that already has the polygon (e.g. wanting to keep it as
    mask_contour) doesn't run findContours twice."""
    if not polygon:
        return None
...
def mask_to_tight_bbox(mask: np.ndarray) -> tuple[float, float, float, float] | None:
    ...
    return polygon_tight_bbox(mask_to_polygon(mask))
